...
def render_kpis(monthly_trend: pd.DataFrame) -> None:
    if monthly_trend.empty:
        st.warning("No sales trend data available yet.")
        return

    latest = monthly_trend.sort_values("period").iloc[-1]
    prev = monthly_trend.sort_values("period").iloc[-2] if len(monthly_trend) > 1 else None
    ...
    col2.metric(
        "Tickets Sold",
        f"{latest['count']:,.0f}",
        count_delta if prev is not None else None,
    )
    col3.metric("Reporting Period", latest["period"].strftime("%b %Y"))


def render_category_chart(summary: pd.DataFrame) -> None:
    if summary.empty:
...
def render_usage_table(alert_table: pd.DataFrame) -> None:
    if alert_table.empty:
        ...
        return
    st.dataframe(
        alert_table.sort_values("projected_depletion_date", na_position="last"),
...
def main() -> None:
    ...
    item_mix = estimate_item_mix(summary)
    usage = estimate_ingredient_usage(item_mix, recipe_book)
    ...
    context_bits = [
        f"Latest revenue: {format_currency(monthly_trend.iloc[-1]['amount'])}"
        if not monthly_trend.empty
        else "Revenue data unavailable.",
        f"Top ingredient at risk: {alert_table.sort_values('days_on_hand').head(1)['ingredient'].iloc[0]}"
        if not alert_table.empty
        else "No ingredient risk computed.",
    ]
